...
export default createConfig({
  ...
  contracts: {
    ...
    /*
     * Arc's shared ERC-8004 registry is deliberately NOT indexed here. It is
     * chain-wide: measured, it emits about 7,000 events per 200k blocks — every
     * agent registered on Arc by anyone — which over this indexer's window is
     * some 40,000 events, each needing its block fetched. Against endpoints that
     * rate-limit per caller, that is not a slow backfill, it is one that never
     * finishes, and it kept the eleven markets this site exists to show stuck at
     * one for days.
     *
     * Our agents' identities are not lost with it: each registers from its own
     * wallet, the agent service records the id it was assigned, and the Agents
     * page reads them from there — with the registration itself verifiable on
     * the explorer, which is the point of a shared registry.
     */
